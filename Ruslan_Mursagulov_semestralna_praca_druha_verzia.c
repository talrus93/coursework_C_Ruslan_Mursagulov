...
void priemer_stlpcev(double **C, int s_1, int r_1)
{
    ...
    int riadok, stlpec;
    double  array_priemer[stlpec], priemer = 0, sum = 0, max_priemer;
    ...
    for (riadok = 0; riadok < s_1; riadok++)
    {
        ...
        array_priemer[riadok] = sum / r_1;


        printf("\n Priemer prvkov stlpca %d : %lf\n", riadok + 1, array_priemer[riadok]);
    }

    printf("\n Stlpec matice C, ktoreho priemer prvkov je najvacsi:  ");
    int max_index = 0;
    max_priemer = array_priemer[0];
    for (riadok = 1; riadok < s_1; riadok++)
    {
        if (array_priemer[riadok] > max_priemer)
        {
            max_priemer = array_priemer[riadok];
            max_index = riadok;
...
int main()
{
    ...
    while(S_1 != R_2)
    {
        printf("pocet stlpcov prvej matice musi rovnat poctu riadkov druhej matice: \n");
        printf("Zadaj rozmer maticy A: " );
        scanf("%d %d", &R_1, &S_1);
        printf("Zadaj rozmer maticy B: " );
        scanf("%d %d", &R_2, &S_2);
    ...
    for (riad_A = 0; riad_A < R_1; riad_A++)
    {
        for (stl_A = 0; stl_A < S_1; stl_A++)
        {
            ...
            while (scanf("%lf",&A[riad_A][stl_A]) != 1)
            {
                scanf("%c", &necislo);
                printf("Zadal ne cislo.Zadaj cisla pre maticy A [%d][%d]: \n",  riad_A, stl_A );
                continue;
            }
    ...
    for (riad_B = 0; riad_B < R_2; riad_B++)
    {
        for (stl_B = 0; stl_B < S_2; stl_B++)
        {
            ...
            while (scanf("%lf",&B[riad_B][stl_B]) != 1)
            {
                scanf("%c", &necislo);
                printf("Zadal ne cislo. Zadaj cisla pre maticy B [%d][%d]: \n",  riad_B, stl_B);
                continue;
            }
